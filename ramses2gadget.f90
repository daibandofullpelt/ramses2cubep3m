...
! is handy.                                                                       !
!                                                                                 !
! IMPORTANT: please read the README.txt before compiling and using this code!!    !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!  Die Benutzung ist ganz einfach: man startet den code mit 2 Argumenten, etwa so:
!
!./ramses2gadget -g ./output_00001
!
!Das erste Argument ist der Modus und das zweite der Ordner mit dem RAMSES
! snapshot der nach GADGET konvertiert werden soll. Drei Modi sind moeglich:
!
!-d
!Der RAMSES snapshot enthaet nur dark matter particles ODER es sollen nur die
!dark matter particles beruecksichtigt werden. In diesem Modus ist der Code ein
!einfacher Format-Konvertierer von RAMSES nach GADGET.
!
!-g
!Der RAMSES snapshot enthaelt zusaetzlich Hydrodynamik-Zellen und eventuell auch
!noch star particles. In diesem Fall wandelt der Code einfach jede AMR-Zelle in
!ein Gasteilchen um, welches dieselbe Masse hat wie das in der Zelle befindliche Gas,
!konvertiert zusaetzlich die star particles (falls vorhanden) und schreibt das Ganze
!im GADGET format raus. Das Problem dabei ist, dass es meistens viel mehr AMR-Zellen
!als dark matter particles gibt, und dementsprechend in dem resultierenden GADGET format
!viel mehr gas particles als dark matter particles, und das koennte u.U. unpraktisch sein.
!
!-i
!Das ist der Interpolation-Modus. Er funktioniert im Prinzip genauso wie -g, nur dass
!nicht mehr ein Particle pro Zelle generiert wird, sondern in Abhaengigkeit von der
!jeweiligen AMR-Aufloesung mehrere AMR-Zellen zu einem Gasparticle zusammengefasst werden.
!Vorteil: Es gibt am Ende ungefähr genauso viele gas particles wie dark matter particles.
!Nachteil: Durch die Interpolierung geht etwas von der Aufloesung verloren.
!
!Ordner: Der Code erwartet dass der RAMSES-Snapshot in einem Ordner wie z.B. output_00001
!liegt und die Dateien dort die folgenden Namen haben:
!
!info_00001.txt
!amr_00001.out00001
...
!...
!
!Der normale RAMSES-Output sieht in der Regel sowieso immer genau so aus.
!
!Der Code kann auch mit MPI kompiliert und parallel laufengelassen werden
!(dann bitte beim Kompilieren -DMPI setzen). In diesem Fall kann man beliebig viele
!RAMSES-Files auf beliebig vielen CPUs konvertieren, der Code kuemmert sich selbst
!um eine sinnvolle Aufteilung.
!
!Achso, noch etwas: das Ganze funktioniert leider nur bis maximal jeweils 1024^3 particles von jeder Sorte.
!
!Noch eine Sache zum Kompilieren: man muss dem Fortran-compiler noch sagen, dass er die
!C-style preprocessor statements wie #define, #ifdef usw. mitnimmt.
!Bei mir funktioniert das mit gfortran -x f95-cpp-input 
